...
**后端**和**数据库**对应文件夹中均有`.local.env.template`文件夹，这个文件会告诉你项目中用到了哪些环境变量（及其格式），但由于环境变量每个人不同、且含有密钥等敏感信息、所以不会上传到 Github 仓库中。
...
**NOTE**: 运行上述命令前，务必配置好`database`目录下的`.local.env`文件。
...
**NOTE**: The above website should be: https://jwtsecrets.com/#generator
...
**NOTE**: The `"<your-secret>"` above means your `JWT_SECRET`.
...
**NOTE**：登陆前，先 `POST /user/register`，然后 `POST /user/login`，两个请求的Body都需要是包含 `username, password` 的字典（选择 `raw`，数据格式选择 `JSON`），例如：

```json
{
    "username": "Test",
    "password": "TTTEEESSSTTT"
}
```

如果成功，则会返回一个包含 `"token"` 的字典，随后我们继续按照上一段进行操作即可。我们可以看到，`user` 表中新增了一行数据，包含 `username, password, uuid`三个键值对。
...
**NOTE**: 我已经完成上述作业。测试步骤如下：
0. 完成环境配置。
1. 用户登陆。或者先注册一个新用户，然后登陆。
2. 在Hasura的控制台中，修改`Authorization`对应的值为`Bearer <Your login-token>`。
3. 在本地`backend`目录通过`yarn start`命令启动项目，随后在Postman中对各路由进行测试。
